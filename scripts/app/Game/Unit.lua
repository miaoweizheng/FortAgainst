...
function Unit:init( node,state,pos )
...
function Unit:hitUnit2()----
	for i,unit2 in ipairs(g_director._listUnit2) do
...
function Unit:toAttack()
	local curpos = self:getPositionInCCPoint()
	local distance
	self._speedX,self._speedY,distance = getSpeedXY(curpos,ccp(display.width, display.height),self._speed)
	-- local angle = getAngle(curpos,ccp(display.width, display.height),distance)
	-- self._img:setRotation(-angle)
end

function Unit:toAlert(  )
